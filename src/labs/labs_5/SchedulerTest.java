...
class Scheduler<T>{
    ...
    public Timestamp<T> next(){
        return timestamps.stream().filter(t -> t.getTime().isAfter(LocalDateTime.now())).min(Timestamp::compareTo).orElse(null);
    }
    public Timestamp<T> last(){
        return timestamps.stream().filter(t -> t.getTime().isBefore(LocalDateTime.now())).max(Timestamp::compareTo).orElse(null);
    }
