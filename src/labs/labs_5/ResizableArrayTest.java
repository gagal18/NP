...
class ResizableArray<T>{
    ...
    public void addElement(T e){
        T[] copyElements = Arrays.copyOf(elements, length+1);
        copyElements[length] = e;
        this.elements = copyElements;
        length++;
    }
    public boolean removeElement(T toRemove) {
        int index = getIndexOfElement(toRemove);
...
class IntegerArray extends ResizableArray<Integer>{
    ...
    public IntegerArray distinct (){
        IntegerArray newIntArr = new IntegerArray();
        for (int i = 0; i < count(); i++) {
            Integer el = elementAt(i);
            if(!newIntArr.contains(el)){
                newIntArr.addElement(el);
            }
        }
        return newIntArr;
    }
    ...
    public IntegerArray increment(int offset) {
        IntegerArray newIntArr = new IntegerArray();
        for(int i = 0; i < count(); i++) {
            newIntArr.addElement(elementAt(i) + offset);
        }
        return newIntArr;
    }
}
