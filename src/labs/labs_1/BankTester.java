...
class Account{
    ...
    public boolean equals(Object obj) {
        ...
        return name.equals(acc.name) && balance.equals(acc.balance) && Id == acc.Id;
    }
}
...
class FlatAmountProvisionTransaction extends Transaction{
    ...
    public double getProvision() {
        return StringToDouble.toDouble(flatProvision);
    }
...
class FlatPercentProvisionTransaction extends Transaction{
    ...
    @Override
    public double getProvision() {
        return (int) StringToDouble.toDouble(getAmount()) * (centsPerDollar / 100.0);
    }
...
class Bank{
    ...
    public boolean makeTransaction(Transaction t) {
        Account accFrom = Arrays.stream(accounts).filter(a -> a.getId() == t.fromID).findFirst().orElse(null);
        Account accTo = Arrays.stream(accounts).filter(a -> a.getId() == t.toID).findFirst().orElse(null);
        if ((accFrom == null) && (accTo == null)) {
            return false;
        }
        double balanceFrom = StringToDouble.toDouble(accFrom.getBalance());
        double transactionAmount = StringToDouble.toDouble(t.getAmount());

        if (balanceFrom < transactionAmount) {
            return false;
        }

        double balanceTo = StringToDouble.toDouble(accTo.getBalance());
        ...
        if (accFrom.equals(accTo)) {
            accFrom.setBalance(String.format("%.2f$", balanceFrom - provisionAmount));
        } else {
            accFrom.setBalance(String.format("%.2f$", balanceFrom - provisionAmount - transactionAmount));
            accTo.setBalance(String.format("%.2f$", balanceTo + transactionAmount));
        }
