...
class Scheduler<T>{
    ...
    public T next(){
        return timestamps.ceilingEntry(new Date()).getValue();
    }
    public T getLast(){
    ...
    public ArrayList<T> getAll(Date begin, Date end) {
        ArrayList<T> allFromRange = new ArrayList<>();
        for (Map.Entry<Date, T> entry: timestamps.entrySet()){
            Date currDate = entry.getKey();
            if(currDate.after(begin) && currDate.before(end)){
                allFromRange.add(entry.getValue());
            }
        }
        return allFromRange;
    }
    ...
    public static void main(String[] args) {
        ...
        if ( k == 4 ) {//test Scheduler with ints complex
            ...
            while ( jin.hasNextLong() ) {
                ...
                Collections.sort(res);
                System.out.println(convertToGMTString(l)+" <: "+print(res)+" >: "+convertToGMTString(h));
            }
