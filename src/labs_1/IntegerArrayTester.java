...
class IntegerArray {
    ...
    public IntegerArray concat(IntegerArray ia){
        int[] enlargedArr = new int[length() + ia.length()];
        for (int i = 0; i < length(); i++) {
            enlargedArr[i] = getElementAt(i);
        }

        for (int i = 0; i < ia.length(); i++) {
            enlargedArr[i + length()] = ia.getElementAt(i);
        }
        return new IntegerArray(enlargedArr);
    }
    @Override
